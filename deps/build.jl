...
if llvm_version >= v"9.0.0"
    run(`$python -m pip install numba llvmlite`)
elseif llvm_version >= v"7.0.0"
    run(`$python -m pip install numba==0.49 llvmlite==0.32`)
elseif llvm_version >= v"6.0.0"
    run(`$python -m pip install numba==0.41 llvmlite==0.26`)
elseif llvm_version >= v"5.0.0"
    run(`$python -m pip install numba==0.38 llvmlite==0.22`)
elseif llvm_version >= v"4.0.0"
    run(`$python -m pip install numba==0.36.2 llvmlite==0.20`)
end